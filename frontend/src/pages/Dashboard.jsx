...
export default function Dashboard() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6" style={{ color: '#e0e0ff' }}>Dashboard</h1>
      ...
      {isSuperAdmin && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 pb-6" style={{ borderBottom: '1px solid #2a2a4a' }}>
          <div className="rounded-xl p-5" style={{ background: '#12121a', border: '1px solid #2a2a4a' }}>
            <p className="text-sm font-medium" style={{ color: '#00f0ff' }}>Total Users</p>
            <p className="text-3xl font-bold mt-1" style={{ color: '#e0e0ff' }}>{stats.totalUsers}</p>
          </div>
          <div className="rounded-xl p-5" style={{ background: '#12121a', border: '1px solid #2a2a4a' }}>
            <p className="text-sm font-medium" style={{ color: '#39ff14' }}>Total Projects</p>
            <p className="text-3xl font-bold mt-1" style={{ color: '#e0e0ff' }}>{stats.totalProjects}</p>
          </div>
          <div className="rounded-xl p-5" style={{ background: '#12121a', border: '1px solid #2a2a4a' }}>
            <p className="text-sm font-medium" style={{ color: '#b026ff' }}>Total Tasks</p>
            <p className="text-3xl font-bold mt-1" style={{ color: '#e0e0ff' }}>{stats.totalTasks}</p>
          </div>
          <div className="rounded-xl p-5" style={{ background: '#12121a', border: '1px solid #2a2a4a' }}>
            <p className="text-sm font-medium" style={{ color: '#ffaa00' }}>Overdue Tasks</p>
            <p className="text-3xl font-bold mt-1" style={{ color: '#e0e0ff' }}>{stats.overdueCount}</p>
          </div>
        </div>
      )}
      ...
      {stats.myTasks.length === 0 && stats.overdueTasks.length === 0 && !isSuperAdmin && (
        <div className="text-center py-16 rounded-xl" style={{ background: '#12121a', border: '1px solid #2a2a4a' }}>
          <p style={{ color: '#666688' }} className="text-lg">No tasks yet. Create a project to get started!</p>
          <Link to="/projects" className="mt-4 inline-block px-6 py-2 rounded-lg transition-all" style={{ background: '#00f0ff', color: '#0a0a0f', boxShadow: '0 0 15px rgba(0, 240, 255, 0.4)' }}>
            Go to Projects
          </Link>
        </div>
      )}
